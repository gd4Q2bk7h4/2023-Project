            ...
            if item["device"] == device_name:
                for i in channel:
                    ...
                    if item["CH"][channel_key] == 'available':
                        item["CH"][channel_key] = str(addr[0])
                    else:
                        return False
                return True
    elif device == 'powermeter':
            ...
            if item["device"] == device_name:
                for i in channel:
                    ...
                    if item["PD"][channel_key] == 'available':
                        item["PD"][channel_key] = str(addr[0])
                    else:
                        return False
                return True
...
def timeout_start(laser_name,laser_channel,powermeter_name,powermeter_channel,laser_dictionary,powermeter_dictionary):
    timer = threading.Timer(device_timout_time, lambda: timeout_operation(laser_name,laser_channel,powermeter_name,powermeter_channel,laser_dictionary,powermeter_dictionary)).start()
