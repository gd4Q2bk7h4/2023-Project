...
def control_Quantifi(request,addr,client_socket,laser_record,laser_dictionary,powermeter_dictionary,Quantifi_laser):
    command = request.replace('QUANTIFI:','')
    print(command)
    if command.startswith('channel_check:'):
        laser_channel_id_string = [command.split(':')[1]]
        laser_channel_id = [ast.literal_eval(laser_channel_id_string[0])]
        to_PC_mes = None
        if query_device_IP('laser','Quantifi',laser_channel_id,addr,laser_dictionary,powermeter_dictionary):
            to_PC_mes = f'Quantifi: CH {laser_channel_id} is a valid device'#match with record'
        elif laser_record[0] == '' and laser_record[1] == []:
            to_PC_mes = f'Quantifi: please request the resource first'
        else:
            to_PC_mes = 'Error: Quantifi channel {} is not a valid channel'.format(laser_channel_id_string) #match'
        response_To_PC(client_socket,to_PC_mes)
    elif command.startswith('query:'):
        command = command.replace('query:','')
        response_To_PC(client_socket,Quantifi_laser.ask(command))
    elif command.startswith('transmit:'):
        command = command.replace('transmit:','')
        print(command)
        response_To_PC(client_socket,Quantifi_laser.write(command))
    elif command.startswith('close'):
        Quantifi_laser.close()