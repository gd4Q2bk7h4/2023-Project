...
device_name_list = {'OSICS':[1,2],'PM32':[i for i in range(16)],'QUANTIFI':[1]}
...
class resource_manager(object):
    ...
    def request_access(self,device='laser',device_name='OSICS',channel_or_channelList=[1]):
        print('\nRequesting access to: {} {}'.format(device_name, channel_or_channelList))
        if device in device_list and device_name in device_name_list:
            if set(channel_or_channelList).issubset(set(device_name_list[device_name])):
                ...
                print(f'invalid channel, pick in {device_name} {device_name_list[device_name]}')
                sys.exit(1)
        else:
            if device not in device_list and device_name not in device_name_list:
                print(f'invalid device, pick in {device_list}')
                print(f'invalid device name, pick in {device_name_list}')
